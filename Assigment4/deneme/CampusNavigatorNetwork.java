...
public class CampusNavigatorNetwork implements Serializable {
    ...
    public List<CartLine> getCartLines(String fileContent) {
        ...
        Pattern p = Pattern.compile(
                "cart_line_name\\s*=\\s*\"([^\"]+)\"[^\\S\\n]*\\n?\\s*cart_line_stations\\s*=\\s*((?:\\(\\d+\\s*,\\s*\\d+\\)\\s*)+)"
        );
        Matcher m = p.matcher(fileContent);
        ...
        while (m.find()) {
            ...
            Pattern stationP = Pattern.compile("\\((\\d+)\\s*,\\s*(\\d+)\\)");
            Matcher stationM = stationP.matcher(stationsRaw);

            int stationCounter = 1;
            while (stationM.find()) {
                int x = Integer.parseInt(stationM.group(1).trim());
                int y = Integer.parseInt(stationM.group(2).trim());
